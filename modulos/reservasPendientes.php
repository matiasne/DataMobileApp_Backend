...
$router->post('/', 'verificarPermiso', function ()  use ($conn) {
	...
	codart_d as codart,PENREM_D as pendiente,cantid_d as cantid,fecemi_c as fecemi 
	...
	WHERE estado_c<>'B' and PENREM_D >0 and TIPOOO_D='FC' and b.pendiente<>0 and clienteid='".$id_cliente."'
	order by fecemi_c";
	...
	if ($num_rows > 0){
		...
		$ret = array(
			'status' => 'succes',
			'code' => '204',
			'data' => 'No se encontraron reservas pendientes'
		);
	}
